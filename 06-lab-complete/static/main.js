...
// API Key (Matches AGENT_API_KEY in .env)
const AGENT_API_KEY = "dev-key-change-me"; 
...
async function sendMessage(text) {
    ...
    try {
        ...
        if (!data || !data.metadata) {
            appendMessage('Security Error: Invalid response from server', 'system');
            return;
        }
...
async function updateMetrics() {
    try {
        ...
        bOutputJudge.style.width = ((data.layer_blocks.OutputGuard || 0) / total * 100) + '%';
