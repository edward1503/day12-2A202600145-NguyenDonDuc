...
def topic_filter(user_input: str) -> bool:
    ...
    # 2. Check for allowed topics (at least one should match)
    for topic in ALLOWED_TOPICS:
        if topic.lower() in input_lower:
            return False

    # 3. If no allowed topics found, block it
    return True
        ...
        *,
        ...
        if detect_injection(text):
            ...
            return self._block_response(
                "I'm sorry, I cannot process this request as it contains potential prompt injection patterns."
            )
        ...
        if topic_filter(text):
            self.blocked_count += 1
            return self._block_response(
                "I'm sorry, but I can only answer questions related to VinBank services and banking topics."
            )
